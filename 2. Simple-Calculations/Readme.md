...
|----------|-----------|
```
 *** Подсказки:** потърсете информация за клас DateTime в PHP и по-конкретно разгледайте функцийте 
strtotime(date . ' + n days') и date(format . ' + n days'). С тяхна помощ може да решите задачата, без да е необходимо да изчислявате дни, месеци и високосни години.
